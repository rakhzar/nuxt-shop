# 🛍️ Nuxt Shop

Интернет-магазин на **Nuxt** + **TypeScript**. Пет-проект для отработки стека: интерфейсы, стейт, интеграция с API.

## 📸 Скриншоты

### 🏠 Главная страница

![Главная страница](/assets/screenshots/main.png)

### 🔐 Вход в систему

![Авторизация](/assets/screenshots/login.png)

### 🛍️ Каталог товаров

![Каталог товаров](/assets/screenshots/shop-catalog.png)
![Каталог товаров 2](/assets/screenshots/shop-catalog-2.png)

## 📖 О магазине

![О магазине](/assets/screenshots/ab1.png)
![О магазине 2](/assets/screenshots/ab2.png)

## 🛠 Стек

- **Framework:** Nuxt 4 (Vue 3)
- **Language:** TypeScript
- **State:** Pinia (+ Persisted State)
- **Styling:** CSS
- **API:** REST, $fetch, useFetch, useDebounceFn
- **Auth:** Token-based (localStorage)
- **Icons:** Nuxt Icon

## 📦 Функционал

- ✅ **Каталог товаров**: отображение карточек, цены, скидки, поиск
- ✅ **Авторизация**: вход по JWT, сохранение токена в localStorage
- ✅ **Безопасный выход (Logout)**: кнопка выхода очищает токен и сбрасывает сессию
- ✅ **Защита роутов**: закрытие личных страниц от неавторизованных пользователей
- ✅ **Избранное**: добавление/удаление товаров (синхронизация с API)
- ✅ **Стейт-менеджмент**: Pinia + Persisted State для сохранения данных

## 🏗 Архитектура

Проект построен по модульному принципу с чётким разделением ответственности. Nuxt работает в **SPA-режиме** (клиентский рендеринг).

```
├── pages/ # Экраны и маршруты (File-based routing)
├── components/ # UI-компоненты (кнопки, карточки, формы)
├── composables/ # Бизнес-логика и работа с API ($fetch, хуки)
├── stores/ # Глобальное состояние (Pinia + Persisted State)
├── interfaces/ # TypeScript типы для API-ответов и форм
├── layouts/ # Общие обёртки страниц (Header, Footer, Nav)
├── middleware/ # Клиентская защита роутов (проверка токена)
└── assets/ # Глобальные стили, шрифты, иконки
```

**Ключевые принципы:**

- 🔹 **TypeScript-first:** Строгая типизация данных через `interfaces/`. Нет `any`, есть контракт.
- 🔹 **Стейт и API:** Вся логика работы с данными инкапсулирована в `stores/` и `composables/`. Компоненты только отображают.
- 🔹 **SPA-режим:** Клиентский рендеринг, быстрая навигация без перезагрузки страницы.
- 🔹 **Чистые компоненты:** UI не знает про API. Данные приходят пропсами или из стора.
- 🔹 **Безопасность роутов:** `middleware/` блокирует доступ к личному кабинету без валидного токена.

## 🚀 Запуск проекта

**Требования:** Node.js 20+ (рекомендуется 22+)

### 1. Клонирование

```bash
git clone https://github.com/rakhzar/nuxt-shop.git
cd nuxt-shop
```

### 2. Установка зависимостей

```bash
npm install
```

### 3. Запуск в режиме разработки

```bash
npm run dev
```

### → Откроется http://localhost:3001
